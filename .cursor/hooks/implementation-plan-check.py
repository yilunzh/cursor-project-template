...
NOTE: Only applies to features that went through ideation and have implementation plans.
"""
import json
import os
import subprocess
...
def get_all_ideation_features(project_dir):
    ...
    ideation_dir = Path(project_dir) / ".cursor" / "ideation"
...
def is_code_file(file_path):
    ...
    # Skip ideation docs
    if ".cursor/ideation" in file_path:
        return False
    # Skip markdown docs (but not AGENTS.md)
    if file_path.endswith(".md") and "AGENTS" not in file_path:
        return False
...
def get_updated_plans(files):
    ...
    updated = []
    for f in files:
        if ".cursor/ideation/" in f and "implementation-plan.md" in f:
...
def main():
    ...
    hook_type = os.environ.get("CURSOR_HOOK_TYPE", "")
    ...
    if plans_needing_update:
        plan_list = "\n".join(
            f"  - .cursor/ideation/{f}/implementation-plan.md"
